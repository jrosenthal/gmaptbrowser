...
function initSettings () {
	...
	for (thisSetting in currSettingsIdx) {
		if (importSettingsIdx[thisSetting]) {  // do I actually need this?  Would the next line cause an error without it?
			settingsObj[thisSetting] = settingsSourceArray[importSettingsIdx[thisSetting]] || getDefaultSetting(thisSetting);		
		} else {
...
console.log('applySetting: ', currSetting, settingsObj[currSetting]);
		switch (currSetting) {
			case 'userCustomSize':
				window.resizeTo(settingsObj[currSetting].split("x")[0],settingsObj[currSetting].split("x")[1]);
				break;
