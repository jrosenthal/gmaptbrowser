...
//		console.log('init',thisSetting,settingsObj[thisSetting]);
if (thisSetting == 'userCustomSize') console.log(thisSetting,settingsObj[thisSetting],settingsSourceArray);
...
console.log('applySetting: ', currSetting, settingsObj[currSetting]);
		switch (currSetting) {
			case 'userCustomSize':
				window.resizeTo(settingsObj[currSetting].split("_")[0],settingsObj[currSetting].split("_")[1]);
				break;
			
			case 'userZoomControl':  // should be irrel as this is referenced directly
				if (settingsObj[currSetting] != "ZOOM_PAN") {
					document.getElementById("zoomPanControl_DOM").checked = true;
				} else {
					document.getElementById("zoomPanControl_DOM").checked = false;
				}
				break; 
...
return;
  var lastApp = settingsObj["userApp"];
  if (lastApp != "IGNORE") {
	  ...
	  thisItem = fc.item().Name;
		...
		if (lastApp != "IGNORE" && lastApp != "NO APP CHOSEN") { // if there was a lastApp, and we actually reach this point (ie:lastApp doesn't match), clear lastApp, and run again.
			settingsObj["userApp"] = "NO APP CHOSEN";
			initializeExternalAppButton();
			}
	  } 
	}	